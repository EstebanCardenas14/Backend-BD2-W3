...
const create = async (req = request, res = response) => {
    const{ rol_id,username,num_doc, doc_id, name, lastname, phone, email, password,estado } = req.body;
   
    try {

        const rolUser = await db.query(`SELECT * FROM rol WHERE id_rol = '${rol_id}'`);
        ...
        await db.query(`INSERT INTO usuario (rol_id, doc_id, username, num_doc, nombres, apellidos, telefono, correo, clave, estado) VALUES ('${rol_id}','${doc_id}','${username}','${num_doc}','${name}','${lastname}','${phone},'${email}','${password}','${estado}')`);
